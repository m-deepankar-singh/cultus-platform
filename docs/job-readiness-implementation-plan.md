...
### 2.1. Core Job Readiness Database Schema
   ...
   - **New Tables to Create:**
     ...
     - [x] `job_readiness_expert_sessions`:
       - Placeholder for Expert Session tracking
       - Fields for student_id, product_id, session_type, completion_status, timestamp
...
### 2.2. Job Readiness API Structure
   ...
   - **Admin Panel API Endpoints:**
     ...
     - [x] `/api/admin/job-readiness/expert-sessions` - Placeholder for Expert Session configuration
   ...
   - **Student App API Endpoints:**
     ...
     - [x] `/api/app/job-readiness/expert-sessions` - Track expert session completion (unlocks Star 3, Projects module, and optionally Promotion Exam).
   ...
   - **Viewer/Client Staff API Endpoints:**
     - [ ] `/api/client-staff/job-readiness/progress` - View student progress for their client
     - [ ] `/api/client-staff/job-readiness/progress/export` - Export progress for their client
...
### 2.3. AI Services Integration
   - **Create New Service Modules:**
     - [ ] `lib/ai/quiz-generator.js` - Service to generate quizzes from video content
     ...
     - [ ] `lib/ai/project-generator.js` - Service to generate projects based on background and tier
       - Specific model to use: Google Gemini Flash 2.0
       - **Background-specific project generation:**
         - [ ] For Computer Science: Generate coding projects requiring GitHub submissions
         - [ ] For other backgrounds: Generate text-based case studies with clear requirements
         - [ ] Uses admin-configured system prompts and input prompts from the `job_readiness_background_project_types` table
       - **Fallback mechanism:**
         - [ ] Maintain a bank of pre-written projects for each background/tier combination
         - [ ] Automatically serve fallback projects if AI generation fails
     - [ ] `lib/ai/project-grader.js` - Service to grade submitted projects
       - **Background-specific grading:**
         - [ ] For Computer Science: Evaluate GitHub repositories (potentially checking for specific functionality)
         - [ ] For other backgrounds: Grade text-based case study responses
     - [ ] `lib/ai/interview-question-generator.js` - Service to generate interview questions
       - Specific model to use: Google Gemini Flash 2.0
       - **Background-specific interview generation:**
         - [ ] Generates different question types based on student's background
         - [ ] Uses admin-configured system prompts and input prompts from the `job_readiness_background_interview_types` table
         - [ ] Adapts difficulty based on student's current tier (Bronze, Silver, Gold)
     - [ ] `lib/ai/video-analyzer.js` - Service to analyze interview videos
       - **Background-specific analysis:**
         - [ ] Uses different evaluation criteria based on student's background
         - [ ] Considers background-specific communication styles and expectations
       - **Enhanced robustness:**
         - [ ] Implement multiple retry attempts with different prompt strategies
         - [ ] Add comprehensive error handling and logging
         - [ ] Include connection health checks before initiating student interviews
     - [ ] `lib/ai/exam-generator.js` - Service to generate promotion exam questions

   - **AI Error Handling and Fallbacks:**
     - [ ] Implement retry logic for AI service calls
     - [ ] Create fallback content if AI services are temporarily unavailable
     - [ ] Log and monitor AI service usage and performance
...
### 3.1. Admin Panel Frontend
   ...
   - **New "Job Readiness Configuration" Section:**
     ...
     - [ ] UI for (placeholder) Expert Session configuration.
...
### 3.2. Student App Frontend
   - **New Student Routes:**
     ...
     - [ ] `/app/job-readiness/expert-sessions` - Expert sessions tracking (for module type `expert_session`).
   ...
   - **Expert Sessions UI (Placeholder):**
     - [ ] This module is locked until Star 2 (Courses completion).
     - [ ] Interface to display/track completion of 5 Expert Sessions.
     - [ ] Integrate with the new `expert_session` module type.
     - [ ] On completion, award the 3rd star. The "Projects" module unlocks.
     - [ ] After Star 3 is achieved, if eligible, present the Promotion Exam as an *optional* step to upgrade tier.
   ...
   - **AI-Powered Submission (Simulated Interview) UI:**
     ...
     - [ ] Display 5 AI-generated interview questions sequentially or all at once.
       - [ ] Questions will be tailored to the student's background (e.g., Computer Science vs Economics)
       - [ ] Emphasize that questions might change on refresh if not yet started.
     - [ ] Display a 4-minute countdown timer.
     - [ ] Video recording controls (start, stop, submit). Auto-submit on timer expiry.
     - [ ] Display AI analysis feedback (pass/fail).
     - [ ] UI to allow retries if failed.
